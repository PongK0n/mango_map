...
function openPackageMenu() {
    ...
    if (isActive) {
        ...
        setTimeout(() => filterMap(currentFilter, true), 10);
        return;
    }
    ...
    const pkgBtn = Array.from(buttons).find(btn => btn.getAttribute('onclick') && btn.getAttribute('onclick').includes('openPackageMenu'));
    if (pkgBtn) pkgBtn.classList.add('active');
}

function closePackagePage() {
    document.getElementById('packageSection').classList.remove('active');
}
...
function selectPackage(index) {
    ...
    if (!isUserLoggedIn) {
        alert("กรุณาเข้าสู่ระบบ (Login) หรือสมัครสมาชิกใหม่ก่อน จึงจะสามารถจองแพ็กเกจได้ครับ");
        toggleAuthModal();
        return;
    }
...
async function submitPackageBooking() {
    ...
    if (error) {
        ...
        alert("🎉 การจองแพ็กเกจสำเร็จ! เจ้าหน้าที่จะติดต่อกลับไปทางเบอร์โทรศัพท์ที่ให้ไว้ครับ");
...
function filterMap(category, isRefresh = false) {
    if (!isRefresh) {
        closePackagePage();
    }
...
// เปิดหน้า Package อัตโนมัติเมื่อเข้าสู่หน้านี้
openPackageMenu();
